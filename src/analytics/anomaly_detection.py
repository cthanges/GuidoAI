...
def detect_rpm_drop(telemetry_df: pd.DataFrame, threshold_drop: float = -1000.0, window_size: int = 5) -> List[Anomaly]:
    anomalies = [] # Get a list of detected anomalies

    # Check if 'nmot' (engine RPM) exists
...
def detect_brake_lockup(telemetry_df: pd.DataFrame, threshold_pressure: float = 80.0, threshold_decel: float = -1.5) -> List[Anomaly]:
    anomalies = []
    ...
    # Get brake pressure (front brake preferred, or else use the rear brake)
...
def detect_speed_anomaly(telemetry_df: pd.DataFrame, max_reasonable_speed: float = 250.0, min_reasonable_speed: float = -5.0) -> List[Anomaly]:
    anomalies = []
    ...
    # Check for unreasonable speed values
...
def detect_performance_drop(telemetry_df: pd.DataFrame, vehicle_id: str, baseline_laps: tuple = (1, 5), check_laps: tuple = (15, 20), threshold_percent: float = 10.0) -> List[Anomaly]:
    anomalies = []
    
    try:
        from src.telemetry_loader import get_vehicle_telemetry
        
        # Get the baseline and check lap speed data
        baseline = get_vehicle_telemetry(telemetry_df, vehicle_id, parameter='Speed', lap_range=baseline_laps)
        check = get_vehicle_telemetry(telemetry_df, vehicle_id, parameter='Speed', lap_range=check_laps)
        
        # Then ensure sufficient data exists
        if len(baseline) < 10 or len(check) < 10:
            return anomalies
        
        # Calculate the 95th percentile speeds for baseline and check laps (this is done to reduce noise in data)
        ...
        # Log anomalies if the drop percent exceeds the threshold
...
def detect_all_anomalies(telemetry_df: pd.DataFrame, vehicle_id: Optional[str] = None, enable_checks: Optional[List[str]] = None) -> Dict[str, List[Anomaly]]:
...
def get_anomaly_summary(anomaly_results: Dict[str, List[Anomaly]]) -> Dict:
    ...
    # Prioritize the most severe anomalies
